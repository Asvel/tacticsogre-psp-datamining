...
var gameTextPrinter = new GameTextPrinter(gameData, 2);
string interpolateGameTexts(FormattableString s) => s.ToString(gameTextPrinter);
...
var sceneNames = deserializer.Deserialize<Dictionary<ushort, string>>(File.ReadAllText(@"..\..\data\sceneNames.yaml"));
...
string formatInstruction(short type, BinaryReader reader)
{
    ...
    var ret = interpolateGameTexts(type switch
    {
        ...
        0x40 => $"IsProtagonistUnarmed {r8():op} {r8() != 0:bo}",
        0x41 => $"IsAnyOurUnitAtBattlestagePosition({r8():!0}{r8()})",  // global/2[x]
        0x42 => $"_UnknownInstruction42(>={r8()}{r8():!0}, {r16():ch})",
        0x43 => $"_UnknownInstruction43(<={r8()}{r8():!0}, {r16():ch})",
...
Dictionary<int, Entry> parseInvk(BinaryReader reader)
{
    ...
    for (int group = 0; group < groupCount; group++)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (instruction0 < 0x60 || instruction0 > 0x6f) throw new InvalidDataException();
...
BinaryReader gameDataReader(uint directoryId, uint fileId, uint childId)
{
    var data = gameData.Load(directoryId, fileId, childId);
...
for (var chapterIndex = 0u; chapterIndex < chapterNames.Length; chapterIndex++)
{
    ...
    var reader = gameDataReader(900, 10 + chapterIndex, 1);
    {
    ...
    // invk and task
    var invk = parseInvk(gameDataReader(900, 10 + chapterIndex, 2));
    var task = parseTask(gameDataReader(900, 10 + chapterIndex, 3));
...
File.WriteAllText(@"..\..\data\screenplays.yaml", serializer.Serialize(pgrsGrouped));

var wrInvkTasks = new Dictionary<uint, object>();
{
    for (var i = 0u; i < 9; i++)
    {
        var invk = parseInvk(gameDataReader(700, 0, 11 + i));
        var task = parseTask(gameDataReader(700, 0, 20 + i));
...
File.WriteAllText(@"..\..\data\wrInvkTasks.yaml", serializer.Serialize(wrInvkTasks));
